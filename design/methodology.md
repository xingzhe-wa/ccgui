...
#### 问题 Q：Tailwind animate-pulse 不适合打字机光标

**根因**：`TypingCursor` 使用了 Tailwind 的 `animate-pulse`（opacity 0→1→0 呼吸效果），但打字机光标应该是"闪烁"而非"呼吸"——在 1s 周期内，前半秒显示，后半秒突然消失，没有渐变过渡。另外 `MessageItem` 也在渲染旧的 `animate-pulse` cursor，导致同一消息同时出现两个光标。

**修复**：
1. 在 `globals.css` 中添加自定义关键帧：
```css
...
```
2. `TypingCursor` 组件改用 `animate-typewriter`
3. `MessageItem` 移除旧的 `animate-pulse` cursor（StreamingMessage 已统一处理）

**教训**：Tailwind 内置动画都是渐变类（ease-in-out），不适合需要"突然闪烁"的场景。始终使用 `step-end` 时序函数定义打字机风格动画。

#### 问题 R：部分 Sprint Task 实际上代码中已完整实现

**根因**：Sprint 7 的 Task 7.1（Skill/Agent列表）、7.2（历史会话搜索）、7.3（多模态输入）在 sprint prompt 文档中被列为"待实现"，但代码审查发现这些功能已完整实现（前端 UI + 后端 handlers）。sprint prompt 与实际代码存在知识 gap。

**修复**：每个 Sprint 开始前，先执行 10 分钟代码考古（code archaeology）：
```bash
# 确认某功能的 handler 是否已存在
grep -r "handleGetSkills\|handleGetAgents" src/main/kotlin

# 确认前端 Store 是否已实现刷新逻辑
grep -r "refreshSkills\|refreshAgents" webview/src

# 确认前端 UI 组件是否已存在
ls webview/src/features/*/components/*Manager*.tsx
```

**教训**：sprint prompt 文档编写前必须先读代码。否则会浪费 Sprint 8 的时间在"重新发现已有实现"上。

### 2.9 Sprint 6 问题与修复

#### 问题 M：highlight.js CSS 主题未导入导致语法高亮失效

**根因**：`CodeBlock.tsx` 导入了 `highlight.js` 库（`import hljs from 'highlight.js'`），并调用 `hljs.highlight()` 生成带 CSS 类名的 HTML。但 `globals.css` 从未导入 highlight.js 的 CSS 主题文件，导致所有 `hljs-*` 类名（如 `hljs-keyword`、`hljs-string`）无样式，代码块显示为纯文本。

**修复**：在 `globals.css` 首行添加：
```css
@import 'highlight.js/styles/github-dark.css';
```

**教训**：JavaScript 库和它的 CSS 样式是独立打包的——引入库不代表引入了样式。前端依赖的 CSS 主题必须显式导入。

#### 问题 N：appStore 与 sessionStore 的 currentSessionId 不同步

**根因**：两套 Store 并行运行（`appStore` 管理会话列表，`sessionStore` 管理各会话的独立状态）。`appStore.switchSession()` 只更新 `appStore.currentSessionId`，不更新 `sessionStore.currentSessionId`。`ChatView` 通过 `sessionStore.sessionStates[sessionStore.currentSessionId]` 读取消息，所以会话切换后读到的仍是旧会话的消息列表。

**修复**：在 `appStore.switchSession()` 中，显式同步到 `sessionStore`：
```ts
...
```
并在 `initializeSessions()` 末尾遍历初始化所有会话的 `sessionStates`：
```ts
...
**教训**：多 Store 架构下，任何改变"当前选中项"的操作必须在所有相关 Store 中同步更新。推荐在 action 内部用 `get().storeB.action()` 调用另一 Store，或使用 Zustand 的 `subscribeWithSelector` middleware 建立联动。

#### 问题 O：AutoResizeTextarea 不支持 ref 转发，导致无法外部聚焦

**根因**：想在 `ChatInput` 中通过 `textareaRef.current?.focus()` 自动聚焦输入框，但 `AutoResizeTextarea` 内部用 `useRef` 创建了私有 ref，外部无法访问。

**修复**：将 `AutoResizeTextarea` 改为 `forwardRef` 组件：
```ts
const AutoResizeTextareaInner = forwardRef<HTMLTextAreaElement, AutoResizeTextareaProps>(
...
**教训**：需要被父组件用 ref 操纵的子组件，必须使用 `forwardRef`。纯内部 ref 的组件在需要外部控制时会无法扩展。

#### 问题 P：TypeScript 泛型箭头函数与 memo/forwardRef 嵌套

**根因**：尝试用 `memo(forwardRef<HTMLTextAreaElement, Props>(function(...){...}))` 内联写法，TypeScript 解析器在行尾 `});` 报告 `error TS1005: ')' expected`。

**修复**：拆分为两步：
```ts
const Inner = forwardRef<HTMLTextAreaElement, Props>(function Component(props, ref) { ... });
export const Component = memo(Inner);
```

**教训**：`memo(forwardRef(fn))` 的内联嵌套写法在某些 TypeScript 版本中解析异常。始终将 `forwardRef` 的结果先用 const 接收，再传给 `memo`。
...
#### 陷阱 13：多 Store 架构下 currentSessionId 跨 Store 不同步

**根因**：`appStore` 和 `sessionStore` 是两个独立的 Zustand store。`appStore.switchSession()` 只更新 `appStore.currentSessionId`，`ChatView` 读取的是 `sessionStore.sessionStates[sessionStore.currentSessionId]`，导致切换会话后 UI 仍显示旧会话数据。

**防御**：任何改变"当前选中项"的状态变更，必须在所有相关 Store 中同步执行。推荐在 action 内部用 `get().otherStoreAction()` 显式同步，不留隐式依赖。

#### 陷阱 14：第三方库的 JS 和 CSS 必须同时引入

`import hljs from 'highlight.js'` 只引入了 JS，不包含 CSS 样式文件。所有 CSS 主题必须独立导入 `import 'highlight.js/styles/xxx.css'`。

**防御**：引入任何 npm 包时，同时确认是否有独立的 CSS/样式文件需要导入。检查 `node_modules/<pkg>/styles/` 目录。

#### 陷阱 15：Sprint Task 声称待实现但代码中已完整存在

**根因**：sprint prompt 文档编写时未充分考古代码，导致 Task 列表中包含已实现的功能。浪费 Sprint 时间在"重新发现"而非真正开发。

**防御**：sprint 开始前先 grep 代码确认关键 handler/UI 是否存在：
```bash
grep -r "handleGetSkills\|handleGetAgents" src/main/kotlin
grep -r "refreshSkills" webview/src
ls webview/src/features/*/components/*Manager*.tsx
```
如 grep 有结果且 Task 确实已完成，标记为 ✅ 已确认，注明"代码考古确认"而非"新实现"。
