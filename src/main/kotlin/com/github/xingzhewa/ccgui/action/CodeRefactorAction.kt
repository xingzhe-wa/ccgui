 ...
 * 代码重构操作
 *
 * 在编辑器中选中代码后，右键菜单 → "重构代码" → 打开 CC Assistant 工具窗口并发送重构请求
 */
class CodeRefactorAction : AnAction() {

    private val log = logger<CodeRefactorAction>()

    override fun actionPerformed(e: AnActionEvent) {
        val project = e.project ?: return
        val editor = e.getRequiredData(CommonDataKeys.EDITOR)
        val selectedText = editor.selectionModel.selectedText?.trim() ?: run {
            log.warn("No text selected for refactoring")
            return
        }

        if (selectedText.isEmpty()) {
            log.warn("Empty text selected for refactoring")
            return
        }
                ...
                bridgeManager.sendMessage(
                    ...
                    callback = object : com.github.xingzhewa.ccgui.bridge.SimpleStreamCallback() {
                        override fun onStreamStart() {
                            log.info("Code refactor request sent")
                        }

                        override fun onStreamError(error: String) {
                            log.warn("Code refactor failed: $error")
                        }
                    }
                )
            } catch (ex: Exception) {
                log.error("Failed to send code refactor request", ex)
            }
        }
    }
