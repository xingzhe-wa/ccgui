package com.github.claudecode.ccgui.ui

import com.github.claudecode.ccgui.application.config.LocaleConfigManager
import com.github.claudecode.ccgui.infrastructure.eventbus.EventBus
import com.github.claudecode.ccgui.infrastructure.eventbus.LocaleChangedEvent
 ...
 */
class LocaleSelectorComponent(private val project: Project) : Disposable {

    private val log = logger<LocaleSelectorComponent>()
    private val localeConfigService = LocaleConfigManager.getInstance(project)

    private val localeComboBox = ComboBox<I18nManager.SupportedLocale>()

    /** EventBus 订阅 ID，用于取消订阅 */
    private var localeSubscriptionId: String? = null
    ...
    private fun setupComboBox() {
        // 填充支持的语言
        val supportedLocales = localeConfigService.getSupportedLocales()
        supportedLocales.forEach { localeComboBox.addItem(it) }

        // 设置当前选中的语言
        val currentLocale = localeConfigService.getCurrentLocale()
        ...
        localeComboBox.addItemListener { event ->
            if (event.stateChange == ItemEvent.SELECTED) {
                ...
                selected?.let {
                    ...
                    localeConfigService.setLocale(it.locale)
                }
            }
        }
 ...
 * 显示当前语言和语言代码
 */
class LocaleStatusComponent(private val project: Project) : Disposable {
    ...
    private fun updateLabel() {
        val locale = localeConfigService.getCurrentLocale()
